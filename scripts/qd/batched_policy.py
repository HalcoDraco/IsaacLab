...
if __name__ == "__main__":
    ...
    # Visual checks
    policy = Policy(obs_dim=2, hidden=4, action_dim=1)
    bp = BatchedPolicy(model=policy, num_policies=3)
    parameters = bp.get_initial_random_parameters(3)
    print("Initial random parameters shape:", parameters.shape)
    print("Initial random parameters:", parameters)

    custom_parameters = torch.tensor(list(range(parameters.numel())), dtype=torch.float32).reshape(parameters.shape)
    print("Custom parameters shape:", custom_parameters.shape)
    print("Custom parameters:", custom_parameters)

    bp.set_population_parameters(custom_parameters)
    print("Population parameters after setting custom values:")
    for name, tensor in bp._population_params.items(): 
        print(f"{name}: shape {tensor.shape}, values:\n{tensor}")
    
    obs = torch.tensor([[0.5, -0.5], [-1.0, 1.0], [0.0, 0.0]])
    out = bp.batched_forward(obs)
    print("Batched forward output shape:", out.shape) 
    print("Batched forward output:", out)